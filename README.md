...
The high conflict rate is expected and by design — 80% of operations target the same 10 hot slots, so most of them are supposed to lose the race. What matters is that they lose it *cleanly*: a **double-booking violation** is two `booking_rooms` rows for the same room with overlapping date ranges — i.e., `commit_booking`/`modify_booking` failed to enforce mutual exclusion under load. Double-booking is now a schema-level impossibility, not just something checked for after the fact: `booking_rooms` carries a GiST exclusion constraint, `booking_rooms_no_overlap` (see `setup_booking_rooms_schema` in [`blue_horizon/load_data/booking_pgsql.py`](blue_horizon/load_data/booking_pgsql.py)), that the database itself enforces against any insert or update, on top of `commit_booking`/`modify_booking`'s own `FOR UPDATE` locking on `room_availability`. `write_ops` catches a hit against that constraint (only reachable if `room_availability` and `booking_rooms` ever drift out of sync) and turns it into the same clean `BookingWriteError` refusal a guest sees for any other unavailable night, rather than leaking a raw database error. (Earlier versions of this stress-test check grouped `room_availability` by `(room_id, date)`, which that table's own `UNIQUE` constraint makes impossible to ever violate — a tautology that always passed regardless of what the agent did. The version above is against `booking_rooms`, the table that can actually represent two guests holding the same night, via the shared [`eval/db_invariants.py`](eval/db_invariants.py) helper the stress test and the eval harness both call — no more separate copies to drift apart; it now stays green by construction rather than by chance, since the constraint above makes a genuine violation unreachable through normal writes, so it functions as a belt-and-suspenders audit rather than the primary guarantee. It was originally confirmed to go red against a hand-inserted overlapping pair once, by hand, before being trusted; that verification became a permanent positive/negative-control regression test — see [`tests/booking/test_db_invariants.py`](tests/booking/test_db_invariants.py) ([history, pinned to the commit before the constraint existed](https://github.com/iannellis/Blue-Horizon-AI-Concierge/blob/e67fb9b33279219a0d4b6b8fc8a85bbb92733dfa/tests/booking/test_db_invariants.py)) — and now tests the constraint's own refusal directly instead, since a hand-inserted overlapping pair can no longer be inserted at all.) A **null-status reservation** is a `room_availability` row without a valid status field, indicating a partially-written or corrupted booking. Separately, the harness reconciles every thread's expected final booking against the database — all 26 threads that ended with a successful book/modify had that exact room and date range sitting `Booked` in the database, confirming the agent never reported a success that didn't actually commit.
