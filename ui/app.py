...
def _init_session_state() -> None:
    ...
    Creates a fresh conversation thread ID and an empty message history.
    """
    if "session_id" not in st.session_state:
        # Stable for the life of the browser session -- unlike thread_id,
        # never regenerated by _reset_session(), since a "New Conversation"
        # click resets the chat thread, not who is holding the guest claim.
...
def _touch_claim(customer_id: int) -> None:
    """Refresh a held claim's activity timestamp.

    Also recreates the claim if it is missing (e.g. after a process
    restart cleared the registry) so a session that survived the restart in
    its own browser tab does not get its guest handed to someone else on
...
def _render_guest_assignment() -> None:
    ...
    holds a claim just has its activity timestamp refreshed here, so an
    idle-but-connected tab does not lose its seat before
    `_GUEST_CLAIM_TTL_S` actually elapses. If every seeded guest is
    claimed, this shows a capacity message instead and leaves
    `customer_id` unset, which the chat input is disabled on.
    """
    customer_id = st.session_state.customer_id
    if customer_id is not None:
        _touch_claim(customer_id)
...
def _handle_sign_out() -> None:
    ...
    rather than left to expire via `_GUEST_CLAIM_TTL_S`.
    """
